...
type HeiaRateLimitGlobals = typeof globalThis & {
  __hayatripsHeiaRateLimitStore?: HeiaRateLimitStore;
};

const RATE_LIMIT_GLOBALS = globalThis as HeiaRateLimitGlobals;

const getStore = (): HeiaRateLimitStore => {
  if (!RATE_LIMIT_GLOBALS.__hayatripsHeiaRateLimitStore) {
    RATE_LIMIT_GLOBALS.__hayatripsHeiaRateLimitStore = new Map();
  }

  return RATE_LIMIT_GLOBALS.__hayatripsHeiaRateLimitStore;
};
