...
export const useAuthScreen = () => {
  ...
  const validateAuthForm = (
    ...
    if (nextStage === "signUp" && !formValues.phone.trim()) {
      return t("authFlow.errors.phoneRequired");
    }
  ...
  const submitSignUp = async () => {
    ...
    try {
      ...
      setSession(nextSession);
      setGuestModeEnabled(false);
      await changeLanguage(formValues.preferredLanguage);
      await upsertCurrentUserProfile({
        firstName: formValues.firstName.trim(),
        phone: formValues.phone.trim(),
        preferredLocale: formValues.preferredLanguage,
      });
      setStage("onboarding");
      setOnboardingStepIndex(0);
