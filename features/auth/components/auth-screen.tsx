...
export const AuthScreen = () => {
  ...
  const languageOptions: {
    ...
    {
      description: t("authFlow.onboarding.options.language.frDescription"),
      icon: "language-outline",
        ...
        <View style={{ gap: spacing.md }}>
          {!isSignIn ? (
            <>
              ...
              <FieldSection label={t("authFlow.fields.phone.label")}>
                <SearchField
                  autoCorrect={false}
                  icon="call-outline"
                  keyboardType="phone-pad"
                  onChangeText={(value) => setFormValue("phone", value)}
                  placeholder={t("authFlow.fields.phone.placeholder")}
                  value={formValues.phone}
                />
              </FieldSection>
              <FieldSection label={t("authFlow.fields.preferredLanguage.label")}>
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
                  {languageOptions.map((option) => (
                    <Chip
                      key={`signup-${option.value}`}
                      label={option.label}
                      onPress={() => {
                        setFormValue("preferredLanguage", option.value);
                        void handlePreferredLanguageChange(option.value);
                      }}
                      selected={formValues.preferredLanguage === option.value}
                      tone={option.value === "ar" ? "coral" : "primary"}
                    />
                  ))}
                </View>
              </FieldSection>
