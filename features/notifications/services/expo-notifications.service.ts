...
const HAYA_TRIP_SCOPE = "hayatrips" as const;
...
type HayaTripsNotificationData = {
  category: ScheduledTravelNotificationCategory | "bookingCreated";
  scheduledFor: string;
...
type NotificationPermissionResponseLike = {
  canAskAgain?: boolean;
  granted?: boolean;
  status?: string | null;
};
...
const getPermissionSnapshotAsync = async (): Promise<PermissionSnapshot> => {
  ...
  try {
    const permissions =
      (await Notifications.getPermissionsAsync()) as NotificationPermissionResponseLike;

    return {
      canAskAgain: permissions.canAskAgain ?? !permissions.granted,
      permissionsStatus: permissions.granted
        ? "granted"
        : mapPermissionStatus(permissions.status),
    };
  } catch {
    return {
      canAskAgain: false,
...
const isHayaTripsNotificationData = (
  value: unknown,
): value is HayaTripsNotificationData => {
  if (!value || typeof value !== "object") {
    return false;
  }

  const candidate = value as Partial<HayaTripsNotificationData>;

  return (
    candidate.scope === HAYA_TRIP_SCOPE &&
    typeof candidate.scheduledFor === "string" &&
    typeof candidate.category === "string" &&
    (candidate.category === "bookingCreated" ||
...
export const scheduleBookingLocalNotificationAsync = async ({
  body,
  bookingId,
  selectedDate,
  title,
  ...
  try {
    ...
    const permissionStatus = await requestLocalNotificationPermissionsAsync();
    ...
    await Notifications.scheduleNotificationAsync({
      ...
      trigger: {
        ...(Platform.OS === "android" ? { channelId: HAYA_TRIP_CHANNEL_ID } : {}),
        seconds: 2,
    ...
    if (selectedDate) {
      await Notifications.scheduleNotificationAsync({
        content: {
          ...
          title: "Haya Trips reminder",
        },
        trigger: {
          ...(Platform.OS === "android" ? { channelId: HAYA_TRIP_CHANNEL_ID } : {}),
          date: ensureFutureDate(withTime(selectedDate, 9, 0), 24),
...
const mapScheduledRequest = (
  ...
  if (!isHayaTripsNotificationData(request.content.data)) {
    return null;
  }

  const data = request.content.data;

  if (data.category === "bookingCreated") {
    return null;
  }
...
export const requestLocalNotificationPermissionsAsync = async () => {
  ...
  try {
    ...
    })) as NotificationPermissionResponseLike;
...
export const buildLocalReminderTemplates = (
  ...
  if (nextTrip) {
    templates.push({
      ...
      deepLink: "/trips",
      id: `hayatrips-trip-reminder-${nextTrip.id}`,
      scope: HAYA_TRIP_SCOPE,
    ...
    templates.push({
      ...
      deepLink: "/trips",
      id: `hayatrips-travel-checklist-${nextTrip.id}`,
      scope: HAYA_TRIP_SCOPE,
  ...
  if (planningTrip) {
    templates.push({
      ...
      deepLink: "/heia",
      id: `hayatrips-ai-prompt-${planningTrip.id}`,
      scope: HAYA_TRIP_SCOPE,
  ...
  if (featuredOffer) {
    templates.push({
      ...
      id: `hayatrips-saved-offer-${featuredOffer.id}`,
      scope: HAYA_TRIP_SCOPE,
  ...
  if (nextTrip) {
    ...
    if (booking) {
      templates.push({
        ...
        id: `hayatrips-booking-milestone-${booking.id}`,
        scope: HAYA_TRIP_SCOPE,
...
export const getHayaTripsScheduledNotificationsAsync = async () => {
  if (!notificationsSupported) {
...
export const cancelHayaTripsScheduledNotificationsAsync = async (
  ...
  const scheduledNotifications =
    await getHayaTripsScheduledNotificationsAsync();
...
export const syncLocalTravelNotificationsAsync = async ({
  ...
  return getHayaTripsScheduledNotificationsAsync();
};
