...
This app is an Expo/React Native client with a small backend route for Haya. The goal is to keep secrets off the client, minimize sensitive persistence on devices, and fail safely when networks or providers are unreliable.
...
- Haya backend responses set `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, and `X-Request-Id`.
- The Haya route only accepts `POST` with a JSON body.
...
- Haya request schemas trim string input and reject unexpected object shapes.
