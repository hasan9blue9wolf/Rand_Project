...
OPENAI_API_KEY=
OPENAI_HEIA_MODEL=gpt-4.1-mini
OPENAI_HEIA_TIMEOUT_MS=12000
HEIA_RATE_LIMIT_MAX_REQUESTS=12
HEIA_RATE_LIMIT_WINDOW_MS=60000
```
...
- `OPENAI_*` and `HEIA_RATE_LIMIT_*` stay on the backend only and must never be exposed to the mobile app.
- Set `EXPO_PUBLIC_HEIA_PROVIDER=real` only after the `/api/heia` backend route is deployed.
